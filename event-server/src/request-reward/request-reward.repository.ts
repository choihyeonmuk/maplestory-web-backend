...
export class RequestRewardRepository {
  constructor(
    @InjectModel(RequestReward.name) private readonly requestRewardModel: Model<RequestReward>,
  ) {}
  ...
  async findAll(queryDto: QueryRequestRewardDto): Promise<RequestReward[]> {
    ...
    return this.requestRewardModel.find(filter).sort({ requestedAt: -1 }).exec();
  }
